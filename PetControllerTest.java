...
public class PetControllerTest {
	...
	public void testDeleteVisit() throws Exception {
		...
		String DESCRIPTION = "Revisión general y vacuna";

		// 1. Crear un DTO de Visita
		VisitDTO newVisitTO = new VisitDTO();
		newVisitTO.setPetId(PET_ID);
		newVisitTO.setDate(LocalDate.parse(DATE));
		newVisitTO.setDescription(DESCRIPTION);

		// 2. Realizar la solicitud POST para crear la visita
		ResultActions mvcActions = mockMvc.perform(post("/visits")
						.content(objectMapper.writeValueAsString(newVisitTO)) // Usa el objectMapper configurado
						.header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON))
				.andDo(print()) // Imprime la solicitud y la respuesta para depuración
				.andExpect(status().isCreated()); // Espera un status 201 (Created)

		// 3. Extraer el ID de la visita creada de la respuesta
		String response = mvcActions.andReturn().getResponse().getContentAsString();
		Integer id = JsonPath.parse(response).read("$.id");

		// 4. Realizar la solicitud DELETE para eliminar la visita por su ID
	...
	public void testDeleteVisitKO() throws Exception {
		...
		mockMvc.perform(delete("/VisitDTO/" + "999" ))
				/*.andDo(print())*/
				.andExpect(status().isNotFound());
	}
